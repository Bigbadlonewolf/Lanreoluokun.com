...
description: "Architecture decision on the Zero Trust signal for BankVault: why MFA freshness (max_age=0 OIDC) gates every grant instead of a standing session token."
---

Knowing who someone is isn't the same as knowing their login is still trustworthy right now. A stolen laptop with a live session can pass a plain identity check. This decision makes BankVault demand a fresh multi-factor login at the moment an underwriter asks to open a credit report, so a stale or hijacked session can't quietly pull sensitive data on the strength of a sign-in that happened hours ago.

> **TL;DR**
>
> - **Decision:** Require a fresh MFA event on every access request (`max_age=0` OIDC, `auth_time` validated against a 15-minute window). Hard deny if stale or missing.
